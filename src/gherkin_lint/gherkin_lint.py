    ...
    def lint_file(self, path: Path) -> List[Dict]:
        ...
        for num, line in enumerate(lines, start=1):
            for check in self.checks:
                errors.extend(check.check(line, num, str(path), context))

        for check in self.checks:
            if hasattr(check, "check_end_of_file"):
                errors.extend(check.check_end_of_file(str(path)))
...
class OutputManager:
    ...
    def print_results(self, file_path: Path, errors: List[Dict]):
        if errors:
            # self.logger.warning(f"\nErrors in {file_path}:")
            for e in errors:
                self.logger.warning(f"{file_path}:{e['line']}: [{e['type']}] {e['message']}")
        else:
            self.logger.info(f"[OK] {file_path}: No linting errors found")
